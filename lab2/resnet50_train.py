...
def save_learning_curve(vgg19_train_accs, vgg19_val_accs):
    ...
    plt.legend()
    plt.savefig('resnet50_accuracy_curve.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    model = ResNet50(100).cuda()

    train_dataset = BufferflyMothLoader(root='dataset', mode='train')
    val_dataset = BufferflyMothLoader(root='dataset', mode='valid')

    train_dataloader = DataLoader(train_dataset, batch_size=batch_size,pin_memory=True,num_workers=16, shuffle=False)
    ...
    for epoch in range(num_epochs):
        ...
        np.savez('resnet50_accuracy.npz', array1 = np.array(train_accs), array2 = np.array(val_accs))